...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \node[block, fill=purple!8, right=1.8cm of bt] (phone) {Android 上位机\\SPP 控制};
...
本人作为本项目的主要负责人，承担了系统方案确定、核心代码实现、联调排错和最终文档整合等主要工作。具体贡献如下：
\begin{enumerate}[leftmargin=2em]
  \item 根据课程要求分析智能手表功能，确定以 STM32F103C8T6、SSD1306、MPU6050、HC-05 和旋转编码器为核心的总体方案，并规划基本功能与扩展功能；
  \item 负责主要硬件接线、模块供电和外设引脚分配，处理 OLED、MPU6050、HC-05、旋转编码器与独立按键之间的连接和调试问题；
  \item 设计并实现 FreeRTOS 多任务程序结构，将时钟维护、输入扫描、传感器采集、OLED 刷新和蓝牙通信拆分为独立任务，并使用互斥量保护共享数据；
  \item 编写和调试 OLED 页面、编码器翻页、PB1 返回表盘、MPU6050 数据读取、姿态显示和基础计步等核心功能；
  \item 负责 HC-05 与 Android 上位机的协议联调，实现同步时间、切换页面、清零步数、请求状态和传感器数据查看等功能；
  \item 统筹最终演示效果，整理 README、检查报告、实物图片、课程设计报告和答辩讲解要点。
\end{enumerate}

在开发过程中，本人投入最多的部分是系统总体设计、嵌入式核心程序开发、硬件联调、蓝牙协议联调和最终材料整合。由于本项目未制作 PCB，硬件部分重点转为保证模块接线稳定、接口说明清晰和现场演示链路完整。

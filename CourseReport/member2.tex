...
本人在项目中主要配合完成传感器、显示效果和实物测试相关工作，在负责人确定总体方案和主要代码框架的基础上，参与了以下内容：
\begin{enumerate}[leftmargin=2em]
  \item 协助整理 OLED、MPU6050、旋转编码器和独立按键的接线关系，参与检查模块供电、共地和 I2C 接线是否正确；
  \item 配合完成 MPU6050 模块调试，参与 WHO\_AM\_I 检测、I2C2 接线排查和 IMU 页面数据观察，帮助确认传感器数据是否稳定；
  \item 参与 OLED 页面显示效果检查，对表盘页、IMU 页、计步页和设备信息页的文字显示、页面切换效果进行实物验证；
  \item 配合测试旋转编码器翻页和 PB1 返回表盘功能，记录旋转方向、按键响应和页面刷新是否存在异常；
  \item 参与整理实物接线图片、OLED 显示图片和功能测试现象，为最终报告中的硬件设计和功能展示部分提供素材；
  \item 协助完成报告中硬件连接、传感器调试和测试结果相关内容的核对。
\end{enumerate}

本人主要承担辅助调试和测试记录工作，重点关注硬件模块连接、MPU6050 数据读取、OLED 页面显示和实物演示现象。项目的总体方案设计、核心程序结构和最终集成由负责人主导完成。

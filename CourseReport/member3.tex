...
本人在项目中主要配合完成蓝牙通信测试、上位机功能验证和验收材料整理工作，在负责人完成主要固件和协议设计后，参与了以下内容：
\begin{enumerate}[leftmargin=2em]
  \item 协助检查 HC-05 与 STM32 USART2 的连接关系，参与确认 PA2/PA3 串口接线、9600 8N1 参数和 STATE 引脚状态；
  \item 参与 Android 上位机连接测试，协助验证手机端连接 HC-05 后的状态显示、时间同步、页面切换和步数清零等操作；
  \item 配合观察蓝牙页面中的连接状态和帧计数变化，记录手机控制命令与 OLED 页面响应是否一致；
  \item 协助完成功能演示流程整理，参与准备表盘显示、IMU 数据、计步页面和蓝牙控制等演示顺序；
  \item 参与收集 Android Studio 界面截图、蓝牙控制演示图和部分测试现象，为报告中的上位机和功能展示部分提供资料；
  \item 协助检查课程报告中的蓝牙通信、Android 上位机、测试环境和不足改进方向等文字内容。
\end{enumerate}

本人主要承担蓝牙链路测试、手机端功能验证和材料整理辅助工作。项目的总体设计、主要固件实现、FreeRTOS 任务划分和最终集成由负责人主导完成。

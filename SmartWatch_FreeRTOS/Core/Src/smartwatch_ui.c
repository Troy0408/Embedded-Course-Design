...
void UI_InitData(SmartWatchData_t *data) {
    ...
    data->current_page = PAGE_WATCH_FACE;
    data->uptime_seconds = 0;
    data->steps = 0;
    data->bt_frames = 0;
    data->step_filtered_g = 1.0f;
    data->step_threshold_g = 1.12f;
    data->mpu_ready_68 = 0;
    data->mpu_ready_69 = 0;
    data->mpu_whoami_68 = 0;
    data->mpu_whoami_69 = 0;
    data->enc_a = 0;
    data->enc_b = 0;
    data->enc_cw = 0;
    data->enc_ccw = 0;
    data->enc_press = 0;
    data->enc_transitions = 0;
    data->imu_status = 0;
    data->bt_connected = 0;
    data->accel.ax = 0.0f;
    data->accel.ay = 0.0f;
...
void UI_DrawIMU(SmartWatchData_t *data) {
    ...
    if (!data->imu_status) {
        ...
        snprintf(line, sizeof(line), "68:%c WHO:%02X",
                 data->mpu_ready_68 ? 'Y' : 'N',
                 (unsigned int)data->mpu_whoami_68);
        OLED_DrawString6x8(4, 5, line);
        snprintf(line, sizeof(line), "69:%c WHO:%02X",
                 data->mpu_ready_69 ? 'Y' : 'N',
                 (unsigned int)data->mpu_whoami_69);
        OLED_DrawString6x8(4, 6, line);
        OLED_DrawString6x8(4, 7, "Check PB10/PB11/VCC");
    } else {
        char buf[32];
        char ax[8], ay[8], az[8];
        char gx[8], gy[8], gz[8];
...
void UI_DrawSteps(SmartWatchData_t *data) {
    ...
    OLED_DrawString8x16(4, 1, "STEPS");

    for (uint8_t x = 0; x < SSD1306_WIDTH; x++)
        OLED_SetPixel(x, 24, 1);

    uint32_t dist_m = (data->steps * 75UL) / 100UL;
    uint32_t cal_x10 = (data->steps * 4UL) / 10UL;
    char acc[8], thr[8], buf[24];

    format_signed_fixed1(acc, sizeof(acc), data->step_filtered_g);
    format_signed_fixed1(thr, sizeof(thr), data->step_threshold_g);

    snprintf(buf, sizeof(buf), "Today: %lu", (unsigned long)data->steps);
    OLED_DrawString6x8(4, 3, buf);

    snprintf(buf, sizeof(buf), "Dist : %lum", (unsigned long)dist_m);
    OLED_DrawString6x8(4, 4, buf);

    snprintf(buf, sizeof(buf), "Cal  : %lu.%lukcal",
             (unsigned long)(cal_x10 / 10UL),
             (unsigned long)(cal_x10 % 10UL));
    OLED_DrawString6x8(4, 5, buf);

    snprintf(buf, sizeof(buf), "Acc%s Thr%s", acc, thr);
    OLED_DrawString6x8(4, 6, buf);

    UI_DrawPageIndicator(PAGE_STEPS, PAGE_MAX);
}
...
void UI_DrawBluetooth(SmartWatchData_t *data) {
    ...
    char frames[24];
    snprintf(frames, sizeof(frames), "Frames: %lu", (unsigned long)data->bt_frames);
    OLED_DrawString6x8(4, 5, frames);
...
void UI_DrawDeviceInfo(SmartWatchData_t *data) {
    ...
    OLED_DrawString6x8(4, 6, "ENC:  PA6/PA7 BTN PB1");

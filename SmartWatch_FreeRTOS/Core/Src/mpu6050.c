...
static uint8_t MPU6050_IsValidWhoAmI(uint8_t whoami)
{
    return (whoami == 0x68U || whoami == 0x70U) ? 1U : 0U;
}
...
uint8_t MPU6050_IsReadyAt(uint8_t addr_7bit)
{
    return (HAL_I2C_IsDeviceReady(&hi2c2, addr_7bit << 1, 1, 20) == HAL_OK) ? 1U : 0U;
}

uint8_t MPU6050_ReadWhoAmIAt(uint8_t addr_7bit)
{
    uint8_t data = 0;
    HAL_StatusTypeDef status = HAL_I2C_Mem_Read(&hi2c2, addr_7bit << 1,
...
int MPU6050_Init(void) {
    ...
    if (!MPU6050_IsValidWhoAmI(whoami)) {
        return -1;  /* Device not found or incompatible ID */
    }
